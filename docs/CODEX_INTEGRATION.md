...
`agent-safe-write` can be used as a deterministic mutation primitive around agent-generated file content. It does not replace Codex's normal repository editing workflow; it is useful when a host or custom agent runtime needs an explicit stale-write and postcondition boundary.
